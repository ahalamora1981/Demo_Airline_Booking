...
init_system_msg = '''###你是一个航空公司订票专员，你的任务是根据用户的输入获取下面“航班预订信息标准格式”中的信息，不要获取其他任何信息。###
###如果信息获取不完整，请继续向用户提问，直到预订信息获取完整为止。###
###如果用户要更改信息，直接更改，不要问”航班预订信息标准格式“之外的信息。###
###获得完整的航班预订信息后，向用户确认预订信息（航班预订信息标准格式），客户确认后感谢客户的预订并结束本次航班预订流程。###
...
st.write("对话历史次数: " + str(len(st.session_state.conversation_history)))
